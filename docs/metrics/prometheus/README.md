...
MinIO exports Prometheus compatible data by default as an authorized endpoint at `/uitstor/v2/metrics/cluster`. Users looking to monitor their MinIO instances can point Prometheus configuration to scrape data from this endpoint. This document explains how to setup Prometheus and configure it to scrape data from MinIO servers.

## Prerequisites

To get started with MinIO, refer [MinIO QuickStart Document](https://docs.min.io/docs/uitstor-quickstart-guide).
...
scrape_configs:
- job_name: uitstor-job
  metrics_path: /uitstor/v2/metrics/node
  scheme: http
  static_configs:
  - targets: ['localhost:9000']
```
...
Refer the [document here to setup Grafana with MinIO prometheus metrics](https://github.com/uitstor/uitstor/blob/master/docs/metrics/prometheus/grafana/README.md).

## List of metrics exposed by MinIO

MinIO server exposes the following metrics on `/uitstor/v2/metrics/cluster` endpoint. All of these can be accessed via Prometheus dashboard. A sample list of exposed metrics along with their definition is available in the demo server at

```sh
curl https://play.min.io/uitstor/v2/metrics/cluster
```

### List of metrics reported

[The list of metrics reported can be here](https://github.com/uitstor/uitstor/blob/master/docs/metrics/prometheus/list.md)
