...
- Install and configure MinIO Server from [here](https://docs.min.io/docs/uitstor-quickstart-guide).
- Install and configure MinIO Client from [here](https://docs.min.io/docs/uitstor-client-quickstart-guide).

```
$ mc admin config get myuitstor | grep notify
...
```sh
$ mc admin config get myuitstor/ notify_amqp
...
Use `mc admin config set` command to update the configuration for the deployment.Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:amqp` at start-up if there were no errors.

An example configuration for RabbitMQ is shown below:

```sh
mc admin config set myuitstor/ notify_amqp:1 exchange="bucketevents" exchange_type="fanout" mandatory="off" no_wait="off"  url="amqp://myuser:mypassword@localhost:5672" auto_deleted="off" delivery_mode="0" durable="off" internal="off" routing_key="bucketlogs"
```
...
MinIO also sends with the notifications two headers: `uitstor-bucket` and `uitstor-event`. An exchange using the type "headers" can use this information to route the notifications to proper queues.
...
We will enable bucket event notification to trigger whenever a JPEG image is uploaded or deleted `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:amqp`. To understand more about ARN please follow [AWS ARN](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) documentation.

```
mc mb myuitstor/images
mc event add myuitstor/images arn:uitstor:sqs::1:amqp --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:amqp s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
```
mc cp myphoto.jpg myuitstor/images
```
...
```py
python rabbit.py
'{"Records":[{"eventVersion":"2.0","eventSource":"aws:s3","awsRegion":"","eventTime":"2016–09–08T22:34:38.226Z","eventName":"s3:ObjectCreated:Put","userIdentity":{"principalId":"uitstor"},"requestParameters":{"sourceIPAddress":"10.1.10.150:44576"},"responseElements":{},"s3":{"s3SchemaVersion":"1.0","configurationId":"Config","bucket":{"name":"images","ownerIdentity":{"principalId":"uitstor"},"arn":"arn:aws:s3:::images"},"object":{"key":"myphoto.jpg","size":200436,"sequencer":"147279EAF9F40933"}}}],"level":"info","msg":"","time":"2016–09–08T15:34:38–07:00"}'
```
...
```sh
$ mc admin config get myuitstor/ notify_mqtt
notify_mqtt:1 broker="" password="" queue_dir="" queue_limit="0" reconnect_interval="0s"  keep_alive_interval="0s" qos="0" topic="" username=""
```

Use `mc admin config set` command to update the configuration for the deployment. Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:mqtt` at start-up if there were no errors.

```sh
mc admin config set myuitstor notify_mqtt:1 broker="tcp://localhost:1883" password="" queue_dir="" queue_limit="0" reconnect_interval="0s"  keep_alive_interval="0s" qos="1" topic="uitstor" username=""
```
...
We will enable bucket event notification to trigger whenever a JPEG image is uploaded or deleted `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:mqtt`.

```
mc mb myuitstor/images
mc event add  myuitstor/images arn:uitstor:sqs::1:mqtt --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:amqp s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
The python program below waits on mqtt topic `/uitstor` and prints event notifications on the console. We use [paho-mqtt](https://pypi.python.org/pypi/paho-mqtt/) library to do this.
...
```sh
$ mc admin config get myuitstor/ notify_elasticsearch
notify_elasticsearch:1 queue_limit="0"  url="" format="namespace" index="" queue_dir=""
```

Use `mc admin config set` command to update the configuration for the deployment. Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:elasticsearch` at start-up if there were no errors.

```sh
mc admin config set myuitstor notify_elasticsearch:1 queue_limit="0"  url="http://127.0.0.1:9200" format="namespace" index="uitstor_events" queue_dir="" username="" password=""
```
...
With the `mc` tool, the configuration is very simple to add. Let us say that the MinIO server is aliased as `myuitstor` in our mc configuration. Execute the following:

```
mc mb myuitstor/images
mc event add  myuitstor/images arn:uitstor:sqs::1:elasticsearch --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:elasticsearch s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
Use curl to view contents of `uitstor_events` index.

```
$ curl  "http://localhost:9200/uitstor_events/_search?pretty=true"
{
  "took" : 40,
  "timed_out" : false,
  "_shards" : {
          ...
          "Records" : [
            {
              ...
              "responseElements" : {
                ...
                "x-uitstor-origin-endpoint" : "http://192.168.86.115:9000"
              },
              "s3" : {
                "s3SchemaVersion" : "1.0",
              ...
              "source" : {
                "host" : "127.0.0.1",
                "port" : "38062",
                "userAgent" : "MinIO (linux; amd64) uitstor-go/2.0.3 mc/2017-02-15T17:57:25Z"
              }
            }
          ]
        }
...
```sh
$ mc admin config get myuitstor/ notify_redis
notify_redis:1 address="" format="namespace" key="" password="" queue_dir="" queue_limit="0"
```

Use `mc admin config set` command to update the configuration for the deployment.Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:redis` at start-up if there were no errors.

```sh
mc admin config set myuitstor/ notify_redis:1 address="127.0.0.1:6379" format="namespace" key="bucketevents" password="yoursecret" queue_dir="" queue_limit="0"
```
...
```
mc mb myuitstor/images
mc event add myuitstor/images arn:uitstor:sqs::1:redis --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:redis s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
1490686879.651061 [0 172.17.0.1:44710] "HSET" "uitstor_events" "images/myphoto.jpg" "{\"Records\":[{\"eventVersion\":\"2.0\",\"eventSource\":\"uitstor:s3\",\"awsRegion\":\"\",\"eventTime\":\"2017-03-28T07:41:19Z\",\"eventName\":\"s3:ObjectCreated:Put\",\"userIdentity\":{\"principalId\":\"uitstor\"},\"requestParameters\":{\"sourceIPAddress\":\"127.0.0.1:52234\"},\"responseElements\":{\"x-amz-request-id\":\"14AFFBD1ACE5F632\",\"x-uitstor-origin-endpoint\":\"http://192.168.86.115:9000\"},\"s3\":{\"s3SchemaVersion\":\"1.0\",\"configurationId\":\"Config\",\"bucket\":{\"name\":\"images\",\"ownerIdentity\":{\"principalId\":\"uitstor\"},\"arn\":\"arn:aws:s3:::images\"},\"object\":{\"key\":\"myphoto.jpg\",\"size\":2586,\"eTag\":\"5d284463f9da279f060f0ea4d11af098\",\"sequencer\":\"14AFFBD1ACE5F632\"}},\"source\":{\"host\":\"127.0.0.1\",\"port\":\"52234\",\"userAgent\":\"MinIO (linux; amd64) uitstor-go/2.0.3 mc/2017-02-15T17:57:25Z\"}}]}"
```

Here we see that MinIO performed `HSET` on `uitstor_events` key.

In case, `access` format was used, then `uitstor_events` would be a list, and the MinIO server would have performed an `RPUSH` to append to the list. A consumer of this list would ideally use `BLPOP` to remove list items from the left-end of the list.
...
To update the configuration, use `mc admin config get` command to get the current configuration file for the uitstor deployment.

```sh
$ mc admin config get myuitstor/ notify_nats
...
```sh
mc admin config set myuitstor notify_nats:1 password="yoursecret" streaming_max_pub_acks_in_flight="10" subject="" address="0.0.0.0:4222"  token="" username="yourusername" ping_interval="0" queue_limit="0" tls="off" streaming_async="on" queue_dir="" streaming_cluster_id="test-cluster" streaming_enable="on"
```
...
We will enable bucket event notification to trigger whenever a JPEG image is uploaded or deleted from `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:nats`. To understand more about ARN please follow [AWS ARN](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) documentation.

```
mc mb myuitstor/images
mc event add myuitstor/images arn:uitstor:sqs::1:nats --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:nats s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
2016/10/12 06:51:33 Received message '{"EventType":"s3:ObjectCreated:Put","Key":"images/myphoto.jpg","Records":[{"eventVersion":"2.0","eventSource":"aws:s3","awsRegion":"","eventTime":"2016-10-12T13:51:33Z","eventName":"s3:ObjectCreated:Put","userIdentity":{"principalId":"uitstor"},"requestParameters":{"sourceIPAddress":"[::1]:57106"},"responseElements":{},"s3":{"s3SchemaVersion":"1.0","configurationId":"Config","bucket":{"name":"images","ownerIdentity":{"principalId":"uitstor"},"arn":"arn:aws:s3:::images"},"object":{"key":"myphoto.jpg","size":56060,"eTag":"1d97bf45ecb37f7a7b699418070df08f","sequencer":"147CCD1AE054BFD0"}}}],"level":"info","msg":"","time":"2016-10-12T06:51:33-07:00"}
```
...
```
Received a message: {"EventType":"s3:ObjectCreated:Put","Key":"images/myphoto.jpg","Records":[{"eventVersion":"2.0","eventSource":"uitstor:s3","awsRegion":"","eventTime":"2017-07-07T18:46:37Z","eventName":"s3:ObjectCreated:Put","userIdentity":{"principalId":"uitstor"},"requestParameters":{"sourceIPAddress":"192.168.1.80:55328"},"responseElements":{"x-amz-request-id":"14CF20BD1EFD5B93","x-uitstor-origin-endpoint":"http://127.0.0.1:9000"},"s3":{"s3SchemaVersion":"1.0","configurationId":"Config","bucket":{"name":"images","ownerIdentity":{"principalId":"uitstor"},"arn":"arn:aws:s3:::images"},"object":{"key":"myphoto.jpg","size":248682,"eTag":"f1671feacb8bbf7b0397c6e9364e8c92","contentType":"image/jpeg","userDefined":{"content-type":"image/jpeg"},"versionId":"1","sequencer":"14CF20BD1EFD5B93"}},"source":{"host":"192.168.1.80","port":"55328","userAgent":"MinIO (linux; amd64) uitstor-go/2.0.4 mc/DEVELOPMENT.GOGET"}}],"level":"info","msg":"","time":"2017-07-07T11:46:37-07:00"}
```
...
>
> ```
> mc admin config set myuitstor/ notify_postgres[:name] connection_string="host=hostname port=2832 username=psqluser password=psqlpass database=bucketevents"
> ```
>
...
Install [PostgreSQL](https://www.postgresql.org/) database server. For illustrative purposes, we have set the "postgres" user password as `password` and created a database called `uitstor_events` to store the events.
...
ARGS:
connection_string*   (string)             Postgres server connection-string e.g. "host=localhost port=5432 dbname=uitstor_events user=postgres password=password sslmode=disable"
...
MINIO_NOTIFY_POSTGRES_CONNECTION_STRING*   (string)             Postgres server connection-string e.g. "host=localhost port=5432 dbname=uitstor_events user=postgres password=password sslmode=disable"
...
```sh
$ mc admin config get myuitstor notify_postgres
notify_postgres:1 queue_dir="" connection_string="" queue_limit="0"  table="" format="namespace"
```

Use `mc admin config set` command to update the configuration for the deployment. Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:postgresql` at start-up if there were no errors.

```sh
mc admin config set myuitstor notify_postgres:1 connection_string="host=localhost port=5432 dbname=uitstor_events user=postgres password=password sslmode=disable" table="bucketevents" format="namespace"
```
...
```
# Create bucket named `images` in myuitstor
mc mb myuitstor/images
# Add notification configuration on the `images` bucket using the MySQL ARN. The --suffix argument filters events.
mc event add myuitstor/images arn:uitstor:sqs::1:postgresql --suffix .jpg
# Print out the notification configuration on the `images` bucket.
mc event list myuitstor/images
mc event list myuitstor/images
arn:uitstor:sqs::1:postgresql s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
...
```
$ psql -h 127.0.0.1 -U postgres -d uitstor_events
uitstor_events=# select * from bucketevents;

key                 |                      value
--------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 images/myphoto.jpg | {"Records": [{"s3": {"bucket": {"arn": "arn:aws:s3:::images", "name": "images", "ownerIdentity": {"principalId": "uitstor"}}, "object": {"key": "myphoto.jpg", "eTag": "1d97bf45ecb37f7a7b699418070df08f", "size": 56060, "sequencer": "147CE57C70B31931"}, "configurationId": "Config", "s3SchemaVersion": "1.0"}, "awsRegion": "", "eventName": "s3:ObjectCreated:Put", "eventTime": "2016-10-12T21:18:20Z", "eventSource": "aws:s3", "eventVersion": "2.0", "userIdentity": {"principalId": "uitstor"}, "responseElements": {}, "requestParameters": {"sourceIPAddress": "[::1]:39706"}}]}
(1 row)
```
...
>
> ```
> mc admin config set myuitstor/ notify_mysql[:name] dsn_string="mysqluser:mysqlpass@tcp(localhost:2832)/bucketevents"
> ```
>
...
Install MySQL from [here](https://dev.mysql.com/downloads/mysql/). For illustrative purposes, we have set the root password as `password` and created a database called `uitstordb` to store the events.
...
```sh
$ mc admin config get myuitstor/ notify_mysql
...
```sh
mc admin config set myuitstor notify_mysql:myinstance table="uitstor_images" dsn_string="root:xxxx@tcp(172.17.0.1:3306)/uitstordb"
```
...
Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::myinstance:mysql` at start-up, if there are no errors.
...
mc mb myuitstor/images
# Add notification configuration on the `images` bucket using the MySQL ARN. The --suffix argument filters events.
mc event add myuitstor/images arn:uitstor:sqs::myinstance:mysql --suffix .jpg
# Print out the notification configuration on the `images` bucket.
mc event list myuitstor/images
arn:uitstor:sqs::myinstance:mysql s3:ObjectCreated:*,s3:ObjectRemoved:*,s3:ObjectAccessed:* Filter: suffix=”.jpg”
```
...
Open MySQL terminal and list the rows in the `uitstor_images` table.

```
$ mysql -h 172.17.0.1 -P 3306 -u root -p uitstordb
mysql> select * from uitstor_images;
+--------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| key_name           | value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
+--------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| images/myphoto.jpg | {"Records": [{"s3": {"bucket": {"arn": "arn:aws:s3:::images", "name": "images", "ownerIdentity": {"principalId": "uitstor"}}, "object": {"key": "myphoto.jpg", "eTag": "467886be95c8ecfd71a2900e3f461b4f", "size": 26, "sequencer": "14AC59476F809FD3"}, "configurationId": "Config", "s3SchemaVersion": "1.0"}, "awsRegion": "", "eventName": "s3:ObjectCreated:Put", "eventTime": "2017-03-16T11:29:00Z", "eventSource": "aws:s3", "eventVersion": "2.0", "userIdentity": {"principalId": "uitstor"}, "responseElements": {"x-amz-request-id": "14AC59476F809FD3", "x-uitstor-origin-endpoint": "http://192.168.86.110:9000"}, "requestParameters": {"sourceIPAddress": "127.0.0.1:38260"}}]} |
+--------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
```sh
$ mc admin config get myuitstor/ notify_kafka
...
Use `mc admin config set` command to update the configuration for the deployment. Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:kafka` at start-up if there were no errors.`bucketevents` is the topic used by kafka in this example.

```sh
mc admin config set myuitstor notify_kafka:1 tls_skip_verify="off"  queue_dir="" queue_limit="0" sasl="off" sasl_password="" sasl_username="" tls_client_auth="0" tls="off" client_tls_cert="" client_tls_key="" brokers="localhost:9092,localhost:9093" topic="bucketevents" version=""
```

### Step 3: Enable Kafka bucket notification using MinIO client

We will enable bucket event notification to trigger whenever a JPEG image is uploaded or deleted from `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:kafka`. To understand more about ARN please follow [AWS ARN](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) documentation.

```
mc mb myuitstor/images
mc event add  myuitstor/images arn:uitstor:sqs::1:kafka --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:kafka s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
    ...
    "Records": [
        {
            ...
            "responseElements": {
                ...
                "x-uitstor-deployment-id": "751a8ba6-acb2-42f6-a297-4cdf1cf1fa4f",
                "x-uitstor-origin-endpoint": "http://192.168.97.83:9000"
            },
            "s3": {
                "s3SchemaVersion": "1.0",
                "configurationId": "Config",
...
ARGS:
endpoint*    (url)       webhook server endpoint e.g. http://localhost:8080/uitstor/events
...
MINIO_NOTIFY_WEBHOOK_ENDPOINT*    (url)       webhook server endpoint e.g. http://localhost:8080/uitstor/events
...
```sh
$ mc admin config get myuitstor/ notify_webhook
...
```sh
mc admin config set myuitstor notify_webhook:1 queue_limit="0"  endpoint="http://localhost:3000" queue_dir=""
```

### Step 2: Enable Webhook bucket notification using MinIO client

We will enable bucket event notification to trigger whenever a JPEG image is uploaded to `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:webhook`. To learn more about ARN please follow [AWS ARN](http://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html) documentation.

```
mc mb myuitstor/images
mc mb myuitstor/images-thumbnail
mc event add myuitstor/images arn:uitstor:sqs::1:webhook --event put --suffix .jpg
```
...
```
arn:uitstor:sqs::1:webhook   s3:ObjectCreated:*   Filter: suffix=".jpg"
```
...
```
mc cp ~/images.jpg myuitstor/images
...
```
mc ls myuitstor/images-thumbnail
...
```sh
$ mc admin config get myuitstor/ notify_nsq
notify_nsq:1 nsqd_address="" queue_dir="" queue_limit="0"  tls="off" tls_skip_verify="off" topic=""
```

Use `mc admin config set` command to update the configuration for the deployment. Restart the MinIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:uitstor:sqs::1:nsq` at start-up if there were no errors.

```sh
mc admin config set myuitstor notify_nsq:1 nsqd_address="127.0.0.1:4150" queue_dir="" queue_limit="0" tls="off" tls_skip_verify="on" topic="uitstor"
```
...
We will enable bucket event notification to trigger whenever a JPEG image is uploaded or deleted `images` bucket on `myuitstor` server. Here ARN value is `arn:uitstor:sqs::1:nsq`.

```
mc mb myuitstor/images
mc event add  myuitstor/images arn:uitstor:sqs::1:nsq --suffix .jpg
mc event list myuitstor/images
arn:uitstor:sqs::1:nsq s3:ObjectCreated:*,s3:ObjectRemoved:* Filter: suffix=”.jpg”
```
