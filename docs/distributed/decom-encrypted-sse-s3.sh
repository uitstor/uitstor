...
./mc encrypt set sse-s3 myuitstor/versioned
...
./mc encrypt info myuitstor/versioned | grep -q "Auto encryption 'sse-s3' is enabled"
ret=$?
if [ $ret -ne 0 ]; then
...
(uitstor server /tmp/xl/{11...30}/disk{0...3} 2>&1 >/tmp/removed.log)&
pid=$!
