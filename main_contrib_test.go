...
// TestRunMain takes arguments from APP_ARGS env variable and calls uitstor.Main(args)
// 1. Build and RUN test executable:
// $ go test -tags testrunmain -covermode count -coverpkg="./..." -c -tags testrunmain
// $ APP_ARGS="server /tmp/test" ./uitstor.test -test.run "^TestRunMain$" -test.coverprofile coverage.cov
//
...
// $ podman run -e MINT_MODE=full -e SERVER_ENDPOINT=192.168.47.11:9000 -e ACCESS_KEY=uitstoradmin -e SECRET_KEY=uitstoradmin -v /tmp/mint/log:/mint/log uitstor/mint
//
...
func TestRunMain(t *testing.T) {
	...
	go func() {
		// start uitstor server with params from env variable APP_ARGS
		args := os.Getenv("APP_ARGS")
		if args == "" {
			log.Printf("No environment variable APP_ARGS found. Starting uitstor without parameters ...")
		} else {
			log.Printf("Starting \"uitstor %v\" ...", args)
		}
		uitstor.Main(strings.Split("uitstor.test "+args, " "))
	}()
	sig := <-cancelChan
