...
func (iamOS *IAMObjectStore) saveIAMConfig(ctx context.Context, item interface{}, objPath string, opts ...options) error {
	...
	if GlobalKMS != nil {
		data, err = config.EncryptBytes(GlobalKMS, data, kms.Context{
			uitstorMetaBucket: path.Join(uitstorMetaBucket, objPath),
		})
		if err != nil {
			return err
		}
...
// Lists files or dirs in the uitstorMetaBucket at the given path
...
func listIAMConfigItems(ctx context.Context, objAPI ObjectLayer, pathPrefix string) <-chan itemOrErr {
	...
	go func() {
		...
		if err := objAPI.Walk(ctx, uitstorMetaBucket, pathPrefix, objInfoCh, ObjectOptions{}); err != nil {
			select {
